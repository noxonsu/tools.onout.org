...
> описываются общие моменты с примерами из Uniswap форка https://github.com/noxonsu/unifactory
...
- зависимое существует из-за уникальных частей. В этих частях нам необходимо автоматизировать (всмысле, чтоб админ мог менять?) изменения, а сам обьем и кол-во таких частей сделать минимальным

2. для начала нужно сократить кол-во зависимостей (в packadge.json ). В примере с Uniswap V2 из зависимого у нас есть множество файлов с импортами `@uniswap/sdk` пакета или констант с адресами и номерами сетей. Сам `@uniswap/sdk` уникален, так как в нем так же есть данные для конкретного блокчейна.
