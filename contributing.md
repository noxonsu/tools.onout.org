...
2. используя блокчейн варианты создаем подобие базы данных, так как нужно будет сохранять разную информацию для каждого из клиентов ([пример](https://github.com/noxonsu/unifactory/blob/main/src/contracts/Registry.sol)). Шаблонная схема: приложение (узнаем текущую сеть пользователя из кошелька) -> адрес контракта для этой сети -> сеть -> контракт -> данные для клиента (к примеру доступ к данным может быть через строку в виде домена или через EVM адрес)
...
4. делаем интерфейс для настроек приложения. Что нужно было поменять для того чтобы форкнуть проект? Как раз для всего этого нужно сделать настройки, которые в итоге можно использовать без изменения кода
